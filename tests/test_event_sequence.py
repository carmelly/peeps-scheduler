...
class TestEventSequenceFinalizationPriorities:
    ...
    def test_finalize_updates_total_attended_for_successful_peeps(
    ...
    def test_finalize_increases_priority_for_unscheduled_responded_peeps(
        self, event_factory, peep_factory
    ):
...
class TestEventSequenceFinalizationMetrics:
    ...
    def test_finalize_calculates_normalized_utilization_correctly(
        self, event_factory, peep_factory
    ):
...
class TestEventSequencePartnerships:
    ...
    def test_calculate_partnerships_fulfilled_counts_mutuals_and_one_sided(
        self, event_factory, peep_factory
    ):
        peep1 = peep_factory(id=1, role=Role.LEADER)
        peep2 = peep_factory(id=2, role=Role.FOLLOWER)
