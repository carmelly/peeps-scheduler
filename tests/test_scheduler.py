...
class TestSchedulerEventSanitization:
    ...
    def test_sanitize_events_keeps_valid_events(self, event_factory, peep_factory):
        ...
        followers = [
            peep_factory(id=i + 11, role=Role.FOLLOWER, availability=[1]) for i in range(4)
        ]
    ...
    def test_sanitize_events_removes_events_without_enough_leaders(
    ...
    def test_sanitize_events_removes_events_without_enough_followers(
        self, event_factory, peep_factory
    ):
        ...
        followers = [
            peep_factory(id=i + 11, role=Role.FOLLOWER, availability=[1]) for i in range(3)
        ]
    ...
    def test_sanitize_events_considers_availability_not_just_role_count(
        self, event_factory, peep_factory
    ):
        ...
        available_leaders = [
            peep_factory(id=i + 1, role=Role.LEADER, availability=[1]) for i in range(3)
        ]  # Only 3 available
        unavailable_leaders = [
            peep_factory(id=i + 11, role=Role.LEADER, availability=[2]) for i in range(5)
        ]  # Not available for event 1
        followers = [
            peep_factory(id=i + 21, role=Role.FOLLOWER, availability=[1]) for i in range(4)
        ]  # Enough followers
        ...
        "valid_events": [
            {
                ...
                "duration_minutes": 90,
                "attendees": [
                    {"id": 1, "name": "John D.", "role": "Leader"},
                    {"id": 2, "name": "Jane S.", "role": "Follower"},
                ],
            },
            {
                "id": 1,
                ...
                "duration_minutes": 90,
                "attendees": [{"id": 1, "name": "John D.", "role": "Leader"}],
            },
        ]
    }
...
def _apply_results(temp_files):
    ...
    scheduler = Scheduler(
        ...
        data_folder=str(temp_files["temp_dir"]),
        max_events=0,
    )
    return scheduler.apply_results()


class TestApplyResultsErrorHandling:
...
class TestRespondedFlagSetting:
    ...
    def test_responded_flag_not_set_for_non_respondents(self, temp_files):
        ...
        assert not jane.responded
        assert not alice.responded
...
class TestPeepIndexOrdering:
    ...
    def test_remove_high_overlap_events_uses_weight_as_tiebreaker(
        self, event_factory, peep_factory
    ):
...
class TestSchedulerSequenceEvaluation:
    ...
    def test_evaluate_sequence_assigns_available_peeps(self, event_factory, peep_factory):
        ...
        followers = [
            peep_factory(id=i + 11, role=Role.FOLLOWER, availability=[1]) for i in range(7)
        ]
        peeps = leaders + followers
    ...
    def test_evaluate_sequence_clears_participants_for_invalid_events(
        self, event_factory, peep_factory
    ):
    ...
    def test_evaluate_sequence_respects_peep_availability(self, event_factory, peep_factory):
        ...
        available_leaders = [
            peep_factory(id=i + 1, role=Role.LEADER, availability=[1]) for i in range(4)
        ]
        unavailable_leader = peep_factory(
            id=10, role=Role.LEADER, availability=[2]
        )  # Not available for event 1

        available_followers = [
            peep_factory(id=i + 11, role=Role.FOLLOWER, availability=[1]) for i in range(4)
        ]
        unavailable_follower = peep_factory(
            id=20, role=Role.FOLLOWER, availability=[2]
        )  # Not available for event 1
        ...
        assert (
            unavailable_leader not in valid_event.leaders
            and unavailable_leader not in valid_event.alt_leaders
        )
        assert (
            unavailable_follower not in valid_event.followers
            and unavailable_follower not in valid_event.alt_followers
        )
    ...
    def test_evaluate_sequence_respects_event_limits(self, event_factory, peep_factory):
        ...
        limited_leader = peep_factory(
            id=1, role=Role.LEADER, availability=[1], event_limit=0
        )  # At limit
        limited_leader.num_events = limited_leader.event_limit  # Simulate already at limit

        unlimited_leaders = [
            peep_factory(id=i + 2, role=Role.LEADER, availability=[1], event_limit=2)
            for i in range(4)
        ]  # Has capacity
        ...
        unlimited_followers = [
            peep_factory(id=i + 11, role=Role.FOLLOWER, availability=[1], event_limit=2)
            for i in range(4)
        ]  # Has capacity
        ...
        assert (
            limited_leader not in valid_event.leaders
            and limited_leader not in valid_event.alt_leaders
        )
        assert (
            limited_follower not in valid_event.followers
            and limited_follower not in valid_event.alt_followers
        )

        # At least some unlimited peeps should be assigned (as attendees or alternates)
        assigned_unlimited_leaders = sum(
            1
            for leader in unlimited_leaders
            if leader in valid_event.leaders or leader in valid_event.alt_leaders
        )
        assigned_unlimited_followers = sum(
            1
            for follower in unlimited_followers
    ...
    def test_evaluate_sequence_adds_alternates_when_event_full(self, event_factory, peep_factory):
        ...
        followers = [
            peep_factory(id=i + 11, role=Role.FOLLOWER, availability=[1]) for i in range(6)
        ]
        peeps = leaders + followers
...
class TestSchedulerDualRoleAssignment:
    ...
    def test_evaluate_sequence_assigns_switch_if_primary_full_to_secondary_role(
        self, event_factory, peep_factory
    ):
    ...
    def test_evaluate_sequence_keeps_primary_only_peeps_in_alternates(
        self, event_factory, peep_factory
    ):
    ...
    def test_evaluate_sequence_ignores_switch_if_needed_in_initial_assignment(
        self, event_factory, peep_factory
    ):
        ...
        switch_if_needed_leader = peep_factory(
            id=10,
            role=Role.LEADER,
    ...
    def test_promote_switch_if_needed_alternate_to_fill_underfilled_role(
        self, event_factory, peep_factory
    ):
    ...
    def test_promote_multiple_switch_if_needed_alternates_if_needed(
        self, event_factory, peep_factory
    ):
    ...
    def test_no_promotion_when_opposite_role_has_no_switch_if_needed(
        self, event_factory, peep_factory
    ):
        ...
        follower_alternate = peep_factory(
            id=20,
            role=Role.FOLLOWER,
            switch_pref=SwitchPreference.PRIMARY_ONLY,
            availability=[1],
    ...
    def test_switch_if_needed_leader_alternate_fills_follower_underfill(
        self, event_factory, peep_factory
    ):
...
class TestSchedulerIntegration:
    ...
    def test_full_scheduling_with_small_dataset(self, event_factory, peep_factory):
        ...
        leaders.append(
            peep_factory(id=1, role=Role.LEADER, availability=[1], priority=3)
        )  # Event 1 only
        leaders.append(
            peep_factory(id=2, role=Role.LEADER, availability=[1], priority=2)
        )  # Event 1 only
        leaders.append(
            peep_factory(id=3, role=Role.LEADER, availability=[1], priority=1)
        )  # Event 1 only
        leaders.append(
            peep_factory(id=4, role=Role.LEADER, availability=[1, 2], priority=2)
        )  # Both events
        leaders.append(
            peep_factory(id=5, role=Role.LEADER, availability=[2], priority=1)
        )  # Event 2 only
        leaders.append(
            peep_factory(id=6, role=Role.LEADER, availability=[2], priority=0)
        )  # Event 2 only
        leaders.append(
            peep_factory(id=7, role=Role.LEADER, availability=[2], priority=1)
        )  # Event 2 only

        # Followers - ensure each event has at least 4 available
        followers.append(
            peep_factory(id=11, role=Role.FOLLOWER, availability=[1], priority=2)
        )  # Event 1 only
        followers.append(
            peep_factory(id=12, role=Role.FOLLOWER, availability=[1], priority=1)
        )  # Event 1 only
        followers.append(
            peep_factory(id=13, role=Role.FOLLOWER, availability=[1], priority=3)
        )  # Event 1 only
        followers.append(
            peep_factory(id=14, role=Role.FOLLOWER, availability=[1, 2], priority=1)
        )  # Both events
        followers.append(
            peep_factory(id=15, role=Role.FOLLOWER, availability=[2], priority=0)
        )  # Event 2 only
        followers.append(
            peep_factory(id=16, role=Role.FOLLOWER, availability=[2], priority=2)
        )  # Event 2 only
        followers.append(
            peep_factory(id=17, role=Role.FOLLOWER, availability=[2], priority=1)
        )  # Event 2 only
