...
from peeps_scheduler.adapters.file.validation.errors import PeriodValidationError
...
def assert_period_error(exc: PeriodValidationError, msg_substring: str) -> None:
    assert msg_substring in str(exc), {
...
class TestValidatePeriod:
    """Integration tests for validate_period with cross-file validation."""

    def test_valid_minimal(self, ctx):
        """Happy path: Minimal valid period data."""
        validated = validate_period(period_data(), ctx.year, ctx.tz)

        assert len(validated["members.csv"].root) == 2
        assert len(validated["responses.csv"].responses) == 2
        assert validated["responses.csv"].event_rows is None
        assert validated["results.json"] is None
        assert validated["actual_attendance.json"] is None
        assert validated["period_config.json"].cancelled_events == []
        assert validated["period_config.json"].cancelled_member_availability == []
        assert validated["period_config.json"].partnership_requests == []
        assert validated["period_config.json"].topics == []
    ...
    def test_response_email_not_found_raises(self, ctx):
        ...
        with pytest.raises(PeriodValidationError) as e:
            validate_period(data, ctx.year, ctx.tz)

        assert_period_error(e.value, "response email")
    ...
    def test_response_from_inactive_member_raises(self, ctx):
        ...
        assert_period_error(e.value, "response from inactive member")
    ...
    def test_valid_topics(self, ctx):
        ...
        validated = validate_period(data, ctx.year, ctx.tz)
        assert validated["period_config.json"].topics == [
    ...
    def test_topics_not_list_raises(self, ctx):
        data = period_data({"period_config.json": {"topics": "Balance for Spins and Turns"}})

        with pytest.raises(PeriodValidationError) as e:
            validate_period(data, ctx.year, ctx.tz)

        errors = e.value.all_errors()
        assert any(
            error.get("file") == "period_config.json" and "topics" in error.get("loc", ())
            for error in errors
        ), errors
    ...
    def test_topics_without_column_raises(self, ctx):
        data = period_data(
            {
                "period_config.json": {"topics": ["Topic A"]},
                "responses.csv": {"responses": [response_data()]},
            }
        )
        with pytest.raises(PeriodValidationError) as e:
            validate_period(data, ctx.year, ctx.tz)
        assert_period_error(e.value, "Deep Dive Topics missing")

    def test_column_without_topics_raises(self, ctx):
        data = period_data(
            {"responses.csv": {"responses": [response_data({"Deep Dive Topics": "Topic A"})]}}
        )
        with pytest.raises(PeriodValidationError) as e:
            validate_period(data, ctx.year, ctx.tz)
        assert_period_error(e.value, "topics missing")
    ...
    def test_topics_with_column_valid(self, ctx):
        ...
        validated = validate_period(data, ctx.year, ctx.tz)
        assert validated["period_config.json"].topics == ["Topic A"]

    def test_no_topics_no_column_valid(self, ctx):
        data = period_data({"responses.csv": {"responses": [response_data()]}})
        validated = validate_period(data, ctx.year, ctx.tz)
        assert validated["period_config.json"].topics == []
    ...
    def test_results_roster_id_not_found_raises(self, ctx):
        ...
        assert_period_error(e.value, "roster id")
    ...
    def test_results_roster_display_name_mismatch_raises(self, ctx):
        ...
        assert_period_error(e.value, "display name")
    ...
    def test_valid_results_roster_name_fallback_to_full_name(self, ctx):
        data = period_data(
            {
                ...
                "responses.csv": {
                    "responses": [response_data({"Email Address": "alice@test.com"})]
                },
                "results.json": {
                    "valid_events": [
                        result_event_data(
        ...
        validated = validate_period(data, ctx.year, ctx.tz)
        assert validated["results.json"] is not None
    ...
    def test_partnership_target_email_not_found_raises(self, ctx):
        ...
        assert_period_error(e.value, "target email")
    ...
    def test_partnership_requester_email_not_found_raises(self, ctx):
        ...
        assert_period_error(e.value, "requester email")
    ...
    def test_cancelled_event_not_found_raises(self, ctx):
        data = period_data(
            {
                ...
                "period_config.json": {"cancelled_events": ["Sunday January 5 - 2pm"]},
            }
        )
        ...
        assert_period_error(e.value, "cancelled event")
    ...
    def test_cancelled_availability_email_not_found_raises(self, ctx):
        ...
        assert_period_error(e.value, "cancelled availability email")
    ...
    def test_cancelled_availability_event_not_found_raises(self, ctx):
        ...
        assert_period_error(e.value, "cancelled availability event")
    ...
    def test_cancelled_availability_event_not_in_member_availability_raises(self, ctx):
        ...
        assert_period_error(e.value, "event not in member's original availability")
    ...
    def test_results_event_not_found_raises(self, ctx):
        ...
        data = period_data(
            {
                ...
                "results.json": {
                    "valid_events": [
                        {
                            "id": 2,
                            "date": "2020-01-05 14:00",  # Sunday 2pm - Not in event_rows
        ...
        assert_period_error(e.value, "result event")
    ...
    def test_attendance_event_not_found_raises(self, ctx):
        ...
        assert_period_error(e.value, "attendance event")
...
class TestFilterResponseTopics:
    ...
    def test_filters_to_matching_topics(self, ctx):
        responses = [
            ResponseCsvRowSchema.model_validate(
                response_data({"Deep Dive Topics": ("Balance for Spins and Turns, Unknown Topic")}),
                context={"ctx": ctx},
            )
        ]
        filter_response_topics(responses, ["Balance for Spins and Turns"])
        assert responses[0].deep_dive_topics == ["Balance for Spins and Turns"]

    def test_filters_to_empty_when_no_topics(self, ctx):
        responses = [ResponseCsvRowSchema.model_validate(response_data(), context={"ctx": ctx})]
        filter_response_topics(responses, [])
        assert responses[0].deep_dive_topics == []
