...
def load_cancelled_events(data_folder, year=None):
    ...
    """
    cancelled_file = Path(data_folder) / "cancelled_events.json"
...
def load_partnerships(data_folder, partnerships_filename="partnerships.json", valid_peep_ids=None):
    ...
    """
    requests_file = Path(data_folder) / partnerships_filename
...
def load_cancellations(filename, year=None):
    ...
    """
    cancellations_file = Path(filename)
    ...
    for event_str in cancelled_event_strings:
        ...
        try:
            ...
            raise ValueError(
                f"cannot parse event string in cancellations.json: '{event_str}' - {e}"
            ) from e
        ...
        for row in rows:
            ...
            for date_str in date_strings:
                try:
                    ...
                    if duration is None:
                        raise ValueError(
                            f"cannot auto-derive event duration from '{date_str}' - time range required"
                        )
                    ...
                    logging.warning(
                        f"Skipping invalid availability string '{date_str}' for {name}: {e}"
                    )
                    continue
...
def parse_event_date(date_str, year=None):
    ...
    if has_time_range:
        ...
        start_time_str, _end_time_str, duration = parse_time_range(time_range_part)
