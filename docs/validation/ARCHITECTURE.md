...
- Schema-to-domain builders (Peep/Event/CancelledMemberAvailability/PartnershipRequest)

**`errors.py`**

- File-level error wrappers and aggregation
...
- Period loading orchestration and `PeriodData` assembly
...
- Period config schemas live here (cancelled events, cancelled member availability, partnership requests)
...
- `events` (derived): Deduplicated `EventSpec` list extracted from event rows or availability
...
- Period config emails must exist in members.csv
...
### 3. Period Config (period_config.json)

**Purpose**: Optional configuration for cancellations and partnership requests.

**Structure** (single file, optional):
...
- `cancelled_events` must be a list of event spec strings
- `cancelled_member_availability` entries require `member_email` + `events`
- `partnership_requests` entries require `requester_email` + `target_emails`
- All emails must be valid and exist in `members.csv`
- Cancelled events must exist in extracted response events
- Cancelled availability events must exist in extracted response events
- Cancelled availability events must be in the member's original availability
- Requester email cannot appear in target emails
- Requester emails must be unique across requests
...
cancelled_member_availability[0].member_email: 'invalid.email' is not valid
  → Must be valid email format

cancelled_member_availability[0].events[1]: event not in member's original availability for alice@example.com
  → Remove the event or update the member's availability

partnership_requests: duplicate requester email in partnerships
  → Each requester can only appear once
```

### 4. Attendance (actual_attendance.json)
...
def load_period(paths, ctx):
    ...
    if paths.period_config:
        with open(paths.period_config) as f:
            period_config = json.load(f)
        payload["cancelled_events"] = period_config.get("cancelled_events", [])
        payload["cancelled_member_availability"] = period_config.get(
            "cancelled_member_availability", []
        )
        payload["partnership_requests"] = period_config.get("partnership_requests", [])
...
4. **Event datetime format consistency** (#036)
   ...
   - Consistent parsing between responses and period_config cancellations

5. **Unknown email references** (#037)
   - Cross-file validation between responses, members, and period_config entries
   - Normalized email comparison (Gmail dot-ignoring)

6. **Cancelled availability consistency**
   - Cancelled events must exist in extracted response events
   - Cancelled availability events must be in the member's original availability
...
- Cancelled availability must be in member availability
- Duplicate requester emails rejected
- Self-partnership rejection

### Integration Tests

- End-to-end PeriodFileSchema validation
- Cross-file validation (responses + members, roster entries, period_config)
